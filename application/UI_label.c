...
void label_draw(uint8_t optional) {
    ...
    //ui.ui_robot_hurt = get_hurt_point();//TODO
    //ui.ui_robot_status = get_robot_status_point();//TODO

    uint8_t temp;
    
    //  !Form: SUPERCAP WARNING
    if (! switch_is_down(get_remote_control_point()->rc.s[0]) && robot_state.current_HP == 0) {
